...
const projects = [
  {
    ...
    description:
      "An application that detects aircraft marshalling signals in real-time using computer vision and deep learning techniques with YOLO object detection.",
  ...
  {
    title: "MERN STACK - Ecommerce Shopping Website",
    description:
      "A classification model to predict income levels based on various demographic and employment factors using decision trees and random forests.",
    image: "MERN Ecommerce Shopping Website",
    tags: ["MongoDb", "React", "Python", "Express", "Nodejs"],
    iconType: "code",
    githubLink: "https://github.com/Saran-Kumar-M/mern-stack-ecommerce",
  ...
  const generateProjectBackground = (name: string, iconType: string) => {
    ...
    const renderIcon = () => {
      switch (iconType) {
        case "ai":
          return (
            <PanelLeftOpen className="h-12 w-12 absolute top-4 right-4 opacity-20 text-white" />
          );
        case "data":
          return (
            <BarChart className="h-12 w-12 absolute top-4 right-4 opacity-20 text-white" />
          );
        case "code":
          return (
            <Code className="h-12 w-12 absolute top-4 right-4 opacity-20 text-white" />
          );
        default:
          return null;
      }
    ...
    return (
      <div
        className="relative w-full h-full bg-gradient-to-br from-opacity-80 to-opacity-100 overflow-hidden group"
        style={{
  ...
  return (
    ...
    >
      <div className="max-w-6xl mx-auto">
        <p className="project-animate text-center text-lg font-semibold text-primary mb-2">
          Browse My Recent
        </p>
        <h2 className="project-animate text-center text-4xl font-bold mb-16">
          Projects
        </h2>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {projects.map((project, index) => (
            <TiltCard key={index} maxTilt={5} glare={true} maxGlare={0.1}>
              <div
                className={`project-animate project-card relative transform-gpu transition-all duration-500 rounded-xl overflow-hidden bg-card shadow-xl h-full border border-border ${
                  hoveredCard === index ? "scale-[1.02] shadow-2xl z-10" : ""
                }`}
